...
const (
	sqlQueryDonationStatAndAccountInfo       = "select temp.id, temp.uid, temp.received_funds, temp.received_supplies, temp.distributed_funds, temp.distributed_supplies, temp.time, temp.nick_name, image.url from (select donation_stat.id, donation_stat.uid, donation_stat.received_funds, donation_stat.received_supplies, donation_stat.distributed_funds, donation_stat.distributed_supplies, donation_stat.created_at as time, account.nick_name from donation_stat full join account on donation_stat.uid = account.id and account.type = ? where donation_stat.created_at >= ? and donation_stat.created_at <= ?) as temp left join image on image.related_id = temp.uid and image.type = ? order by temp.time desc limit ? offset ?"
