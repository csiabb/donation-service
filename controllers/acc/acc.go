...
func (h *RestHandler) Login(c *gin.Context) {
	...
	if req.Phone != "" && !utils.CheckPhone(req.Phone) {
		logger.Errorf("invalid params phone: %s", req.Phone)
		c.JSON(http.StatusBadRequest, rest.ErrorResponse(rest.InvalidParamsErrCode, "invalid phone number"))
		return
	}

	if req.Password != "" && !utils.CheckPassword(req.Password) {
		logger.Errorf("invalid params password: %s", req.Password)
		c.JSON(http.StatusBadRequest, rest.ErrorResponse(rest.InvalidParamsErrCode, "invalid password"))
		return
	}
	...
	user, err := h.srvcContext.DBStorage.CheckAccount(req.Phone, wxCredentials.OpenID)
	if err != nil {
