...
func Init(t *testing.T) (*gomock.Controller, *RestHandler, *mock_backend.MockIDBBackend, *mock_wx.MockIWXClient, *httptest.ResponseRecorder, *gin.Context) {
	...
	return mockCtl, &handler, mockDB, mockWX, w, c
}

func TestLoginWXAppUserExist(t *testing.T) {
	mockCtl, handler, mockDB, mockWX, w, c := Init(t)
	...
	mockDB.EXPECT().CheckAccount(gomock.Any()).Return(acc, nil)
...
func TestLoginWXAppWXResp(t *testing.T) {
	mockCtl, handler, _, mockWX, w, c := Init(t)
	defer mockCtl.Finish()
...
func TestLoginWXAppCreateAccount(t *testing.T) {
	...
	mockWX.EXPECT().WXLogin(gomock.Any(), gomock.Any(), gomock.Any()).Return(lr, nil)
	mockDB.EXPECT().CheckAccount(gomock.Any()).Return(nil, gorm.ErrRecordNotFound)
